...
func RegisterAttendanceLogModule(router *gin.RouterGroup, db *gorm.DB) {
	...
	group.Use(cryptography.JWTMiddleware(db))
	{
		...
		group.POST("/bulk", permissions.Handler("attendance-logs", "create"), controller.CreateBulk)
		...
		group.DELETE("/:id/archived", permissions.Handler("attendance-logs", "archive"), controller.Archive)
